...
Option 1: Florida Parcels 

Let's figure out what census tracts the single family Florida Homeowners in `pinellas_parcels.shp` are in. `pinellas_tracts.geojson` has census tract data for total population and the Black population. 
...
### Load in pinellas parcels data
st_read()

```

Option 2: CCTV Cameras
